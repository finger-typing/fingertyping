export const wordLists: { [key: string]: string[] } = {
  English: [
    // Shorter words (3 to 5 characters)
    "go,", // Go
    "To", // To
    "my", // My
    "for", // For
    "the", // The
    "of", // Of
    "cat", // Cat
    "dog.", // Dog
    "book", // Book
    "water", // Water
    "home", // Home
    "city", // City
    "apple", // Apple
    "ball", // Ball
    "cup", // Cup
    "doll", // Doll
    "egg", // Egg
    "fan", // Fan
    "gift,", // Gift
    "hat", // Hat
    "ice", // Ice
    "jump", // Jump
    "kite", // Kite
    "love", // Love
    "moon", // Moon
    "nest", // Nest
    "owl", // Owl
    "pen", // Pen
    "quiz", // Quiz
    "rain", // Rain
    "star", // Star
    "toy", // Toy
    "up", // Up
    "van", // Van
    "wind", // Wind
    "xray", // X-ray
    "yarn", // Yarn
    "zero", // Zero
    "food", // Food
    "lamp", // Lamp
    "desk", // Desk
    "door", // Door
    "tree", // Tree
    "fish", // Fish
    "hand.", // Hand
    "road", // Road
    "sun", // Sun
    "sky", // Sky
    "car", // Car
    "shoe", // Shoe
    "bike", // Bike
    "milk", // Milk
    "an",
    "a", // A
    "the", // The
    "I",
    "you", //you
    "she",
    "he",
    "it",
    "they",
    "them",
    "holiday", // Holiday
    "friends", // Friends
    "his",
    "her",
    "fire",
    "Dubai",
    "Lahore",
    "new",
    "old",
    "good",
    "bad",
    "library", // Library
    "station", // Station
    "solution.", // Solution
    "great",
    "small",
    "large",
    "beautiful",
    "one",
    "two",
    "three",
    "balance", // Balance
    "meeting", // Meeting
    "Barishal.", // Barishal
    "our",
    "your",
    "his",

    // Longer words (6 to 8 characters)
  ],
  Bangla: [
    // Longer words (6 to 8 characters)

    "বিশ্বাস", // Trust
    "ভাষা", // Language
    "তথ্য", // Information
    "স্বপ্ন", // Dream
    "আকাশ", // Sky
    "সূর্য", // Sun
    "চাঁদ", // Moon
    "তারা", // Star
    "সমুদ্র", // Sea
    "নদী", // River
    "পাহাড়", // Mountain
    "গাছ", // Tree
    "ফুল", // Flower
    "ফল", // Fruit
    "পাখি", // Bird
    "মাছ", // Fish
    "বাঘ", // Tiger
    "হাতি", // Elephant
    "ঘর", // House
    "স্কুল", // School
    "বাজার", // Market
    "খাবার", // Food
    "পানি", // Water
    "আগুন", // Fire
    "নতুন", // New
    "সামাজিক", // Social
    "মোবাইল", // Mobile
    "সফলতা", // Success
    "কৌশল", // Strategy
    "উপহার", // Gift

    // Shorter words (3 to 5 characters)

    "পানি", // Water
    "গাছ", // Tree
    "ফুল", // Flower
    "মাঠ", // Field
    "বই", // Book
    "চা", // Tea
    "ছবি", // Picture
    "ছাতা", // Umbrella
    "কাঠ", // Wood
    "দরজা", // Door
    "মাটি", // Soil
    "বাড়ি", // Home
    "রুটি", // Bread
    "পাঠ", // Lesson
    "মাছ", // Fish
    "লাল", // Red
    "পেনসিল", // Pencil
    "নদী", // River
    "রঙ", // Color
    "লেবু", // Lemon
    "খাবার", // Food
    "পাতা", // Page
    "পাত্র", // Cup
    "সুবিধা", // Convenience
    "স্বাস্থ্য", // Health
    "বাজার", // Market
    "সাহিত্য", // Success
    "প্রশ্ন", // Question
    "অন্যান্য", // Other
    "সম্পর্ক", // Relationship
    "সমাজ", // Culture
    "সম্প্রদায়", // Community
    "সম্পাদনা", // Editing
    "সম্পূর্ণ", // Complete
    "সম্বন্ধ", // Relation
    "আমি", // I
    "তুমি", // You
    "আমাদের", // Our
    "তাদের", // Their
    "আমাকে", // Me
    "তোমাকে", // You
    "আমার", // My
    "তার", // Your
    "তোমার", // Your
    "আমাকে", // Our
  ],
  Arabic: [
    // Longer words (6 to 8 characters)
    "تعليم", // Education
    "اقتصاد", // Economics
    "علم النفس", // Psychology
    "مالية", // Finance
    "تكنولوجيا", // Technology
    "تنظيم", // Organization
    "بحث علمي", // Research
    "تطوير", // Development
    "جامعة", // University
    "معلومات", // Information
    "مجتمع", // Society
    "ممارسة", // Practice
    "مشروع", // Project
    "إنترنت", // Internet
    "نشر", // Publication
    "سؤال", // Question
    "مؤسسة", // Enterprise
    "احتياجات", // Needs
    "عقد", // Contract
    "وثيقة", // Document
    "إصلاح", // Reform
    "إعلانات", // Advertising
    "نقل", // Transport
    "أخصائي", // Specialist
    "نظام", // System
    "تغييرات", // Changes
    "تحضير", // Preparation
    "نشاط", // Activity
    "شراكة", // Partnership
    "تجربة", // Experience
    "تخطيط", // Planning
    "استراتيجية", // Strategy
    "تطوير", // Development
    "إنتاج", // Production

    // Shorter words (3 to 5 characters)
    "قط", // Cat
    "كلب", // Dog
    "كتاب", // Book
    "ماء", // Water
    "بيت", // House
    "مدينة", // City
    "طعام", // Food
    "مصباح", // Lamp
    "طاولة", // Table
    "باب", // Door
    "شجرة", // Tree
    "سمك", // Fish
    "يد", // Hand
    "شارع", // Street
    "شمس", // Sun
    "سماء", // Sky
    "سيارة", // Car
    "حذاء", // Shoe
    "دراجة", // Bike
    "حليب", // Milk
  ],
  French: [
    // Longer words (6 to 8 characters)
    "ordinateur", // Computer
    "éducation", // Education
    "économie", // Economics
    "psychologie", // Psychology
    "finance", // Finance
    "technologie", // Technology
    "société", // Society

    // Shorter words (3 to 5 characters)
    "chat", // Cat
    "chien", // Dog
    "livre", // Book
    "eau", // Water
    "maison", // House
    "ville", // City
    "nourriture", // Food
    "lampe", // Lamp
    "bureau", // Desk
    "porte", // Door
    "arbre", // Tree
    "poisson", // Fish
    "main", // Hand
    "route", // Road
    "soleil", // Sun
    "ciel", // Sky
    "voiture", // Car
    "chaussure", // Shoe
    "vélo", // Bike
    "lait", // Milk
    "papier", // Paper
    "pierre", // Iron
    "feuille", // Leaf
  ],

  Spanish: [
    // Longer words (6 to 8 characters)

    "sociedad", // Society
    "práctica", // Practice
    "proyecto", // Project
    "internet", // Internet
    "pregunta", // Question
    "empresa", // Enterprise
    "necesidades", // Needs
    "contrato", // Contract
    "documento", // Document
    "reforma", // Reform

    // Shorter words (3 to 5 characters)
    "gato", // Cat
    "perro", // Dog
    "libro", // Book
    "agua", // Water
    "casa", // House
    "ciudad", // City
    "comida", // Food
    "lámpara", // Lamp
    "mesa", // Table
    "puerta", // Door
    "árbol", // Tree
    "pez", // Fish
    "mano", // Hand
    "calle", // Street
    "sol", // Sun
    "cielo", // Sky
    "coche", // Car
    "zapato", // Shoe
    "bicicleta", // Bike
    "leche", // Milk
    "teléfono", // Phone
    "papel", // Paper
    "piso", // Floor
    "puerta", // Door
    "árbol", // Tree
    "pez", // Fish
    "mano", // Hand
    "calle", // Street
    "sol", // Sun
    "cielo", // Sky
    "coche", // Car
    "zapato", // Shoe
    "bicicleta", // Bike
    "leche", // Milk
  ],

  //start hindi word
  Hindi: [
    // Longer words (6 to 8 characters)
    "संगठन", // Organization
    "विज्ञान", // Science
    "अनुभव", // Experience
    "समाधान", // Solution
    "शिक्षा", // Education
    "उद्योग", // Industry
    "संस्कृति", // Culture
    "आवेदन", // Application
    "राजधानी", // Capital
    "स्वास्थ्य", // Health
    "साहित्य", // Literature
    "स्वतंत्रता", // Freedom
    "समय", // Time
    "समाचार", // News
    "परिस्थिति", // Situation
    "अवसर", // Opportunity
    "विकास", // Development
    "मान्यता", // Recognition
    "प्रस्तावना", // Introduction
    "उपलब्धि", // Achievement
    "विवाह", // Marriage
    "संकल्प", // Resolution
    "प्रयोगशाला", // Laboratory
    "उदाहरण", // Example
    "संतुलन", // Balance
    "व्यापार", // Business
    "समय", // Time
    "उपहार", // Gift

    // Shorter words (3 to 5 characters)
    "सेब", // Apple
    "कुर्सी", // Chair
    "पानी", // Water
    "घर", // Home
    "किताब", // Book
    "फल", // Fruit
    "गांव", // Village
    "रोटी", // Bread
    "मास", // Month
    "फूल", // Flower
    "कागज", // Paper
    "बिल", // Bill
    "काम", // Work
    "मिट्टी", // Soil
    "आम", // Mango
    "पेन", // Pen
    "सूरज", // Sun
    "मछली", // Fish
    "चाय", // Tea
    "नदी", // River
    "बाजार", // Market
    "जांच", // Inspection
    "बाजार", // Market
  ],

  //start urdu
  Urdu: [
    "کتاب", // Book (simple)
    "سیب", // Apple (simple)
    "پودا", // Plant (simple)
    "گلاس", // Glass (simple)
    "میز", // Table (simple)
    "پانی", // Water (simple)
    "پھول", // Flower (simple)
    "درخت", // Tree (simple)
    "گھر", // House (simple)
    "پھل", // Fruit (simple)
    "کھانا", // Food (simple)
    "چائے", // Tea (simple)
    "کرسی", // Chair (simple)
    "قلم", // Pen (simple)
    "کترا", // Piece (simple)
    "موٹر", // Motor (simple)
    "گلاب", // Rose (simple)
    "لباس", // Clothing (simple)
    "پینسل", // Pencil (simple)
    "خوشبو", // Fragrance (complex)
    "تختہ", // Board (simple)
    "چمچ", // Spoon (simple)
    "مکان", // Home (simple)
    "بازار", // Market (complex)
    "کچن", // Kitchen (simple)
    "پرندہ", // Bird (complex)
    "چھوٹا", // Small (simple)
    "دکان", // Shop (simple)
    "عید", // Festival (simple)
    "پٹاخہ", // Firecracker (complex)
    "خدا", // God (simple)
    "مچھلی", // Fish (simple)
    "سفر", // Journey (simple)
    "لباس", // Dress (simple)
    "پیار", // Love (simple)
    "گھڑی", // Watch (simple)
    "سرف", // Detergent (complex)
    "موٹر", // Motor (simple)
    "آگ", // Fire (simple)
    "پتھر", // Stone (simple)
    "کھلونا", // Toy (simple)
    "شجر", // Tree (complex)
    "عید", // Eid (simple)
    "بازار", // Market (complex)
    "پینٹ", // Paint (simple)
    "کمپیوٹر", // Computer (complex)
    "صوت", // Sound (complex)
    "بچوں", // Children (simple)
    "ہوا", // Air (simple)
    "کھڑکی", // Window (complex)
  ],
  //Mandarin words

  Chinese: [
    "组织结构", // zǔzhī jiégòu (Organization structure)
    "经济学", // jīngjìxué (Economics)
    "教育系统", // jiàoyù xìtǒng (Education system)
    "政府机关", // zhèngfǔ jīguān (Government agency)
    "科学研究", // kēxué yánjiū (Scientific research)
    "国际关系", // guójì guānxì (International relations)
    "环境保护", // huánjìng bǎohù (Environmental protection)
    "公共服务", // gōnggòng fúwù (Public service)
    "社会问题", // shèhuì wèntí (Social issues)
    "历史文化", // lìshǐ wénhuà (Historical culture)
    "文化遗产", // wénhuà yíchǎn (Cultural heritage)
    "企业管理", // qǐyè guǎnlǐ (Business management)
    "政策制定", // zhèngcè zhìdìng (Policy formulation)
    "金融市场", // jīnróng shìchǎng (Financial market)
    "科研成果", // kēyán chéngguǒ (Research achievements)
    "市场经济", // shìchǎng jīngjì (Market economy)
    "道德规范", // dàodé guīfàn (Ethical norms)
    "生活方式", // shēnghuó fāngshì (Lifestyle)
    "社会服务", // shèhuì fúwù (Social services)
    "人际关系", // rénjì guānxì (Interpersonal relationships)
    "技术创新", // jìshù chuàngxīn (Technological innovation)
    "政治学", // zhèngzhìxué (Political science)
    "经济发展", // jīngjì fāzhǎn (Economic development)
    "社会学", // shèhuìxué (Sociology)
    "公共管理", // gōnggòng guǎnlǐ (Public administration)
    "健康保健", // jiànkāng bǎojiàn (Health care)
    "环境管理", // huánjìng guǎnlǐ (Environmental management)
    "文化交流", // wénhuà jiāoliú (Cultural exchange)
    "自然资源", // zìrán zīyuán (Natural resources)
    "国际贸易", // guójì màoyì (International trade)
    "教育改革", // jiàoyù gǎigé (Education reform)
    "领导力", // lǐngdǎo lì (Leadership)
    "互联网", // hùliánwǎng (Internet)
    "知识产权", // zhīshì chǎnquán (Intellectual property)
    "数据分析", // shùjù fēnxī (Data analysis)
    "社会网络", // shèhuì wǎngluò (Social network)

    // Shorter words (3 to 5 characters)
    "苹果", // píngguǒ (Apple)
    "桌子", // zhuōzi (Table)
    "水", // shuǐ (Water)
    "家", // jiā (Home)
    "书", // shū (Book)
    "花", // huā (Flower)
    "车", // chē (Car)
    "食物", // shíwù (Food)
    "杯子", // bēizi (Cup)
    "电脑", // diànnǎo (Computer)
    "牛奶", // niúnǎi (Milk)
    "电视", // diànshì (TV)
    "公园", // gōngyuán (Park)
    "手机", // shǒujī (Phone)
    "雨", // yǔ (Rain)
    "窗户", // chuānghù (Window)
    "餐厅", // cāntīng (Restaurant)
    "海", // hǎi (Sea)
    "蛋", // dàn (Egg)
    "猫", // māo (Cat)
  ],

  //indodesian words

  Indonesian: [
    // Longer words (6 to 8 characters)
    "komputer", // Computer
    "kesehatan", // Health
    "perusahaan", // Company
    "pendidikan", // Education
    "sistematis", // Systematic
    "penelitian", // Research
    "pertanian", // Agriculture
    "pembangunan", // Development
    "politik", // Politics
    "perdagangan", // Trade
    "pengobatan", // Medicine
    "lingkungan", // Environment
    "manajemen", // Management
    "komersial", // Commercial
    "asuransi", // Insurance
    "pembayaran", // Payment
    "transaksi", // Transaction
    "strategi", // Strategy
    "inovasi", // Innovation
    "pelayanan", // Service
    "penerjemah", // Translator
    "perubahan", // Change
    "sumberdaya", // Resources

    // Shorter words (3 to 5 characters)
    "air", // Water
    "buku", // Book
    "meja", // Table
    "kursi", // Chair
    "pintu", // Door
    "jalan", // Street
    "gula", // Sugar
    "kopi", // Coffee
    "makan", // Eat
    "buah", // Fruit
    "mobil", // Car
    "lampu", // Lamp
    "dari", // From
    "ibu", // Mother
    "ayah", // Father
    "mata", // Eye
    "pohon", // Tree
    "baju", // Shirt
    "roti", // Bread
    "susu", // Milk
  ],

  //russian words

  Russian: [
    // Longer words (6 to 8 characters)
    "компьютер", // komputer (Computer)
    "образование", // obrazovanie (Education)
    "наука", // nauka (Science)
    "экономика", // ekonomika (Economics)
    "государство", // gosudarstvo (State)
    "организация", // organizatsiya (Organization)
    "исследование", // issledovaniye (Research)
    "технология", // tekhnologiya (Technology)
    "специалист", // spetsialist (Specialist)
    "проект", // proyekt (Project)
    "развитие", // razvitie (Development)
    "университет", // universitet (University)
    "информация", // informatsiya (Information)
    "площадь", // ploshchad' (Area)
    "учебник", // uchebnik (Textbook)
    "реклама", // reklama (Advertising)
    "документ", // dokument (Document)

    // Shorter words (3 to 5 characters)
    "мир", // mir (World)
    "дом", // dom (House)
    "снег", // sneg (Snow)
    "солнце", // solntse (Sun)
    "вода", // voda (Water)
    "книга", // kniga (Book)
    "стол", // stol (Table)
    "кошка", // koshka (Cat)
    "собака", // sobaka (Dog)
    "сайт", // sayt (Site)
    "день", // den' (Day)
    "ночь", // noch' (Night)
    "папа", // papa (Dad)
    "мама", // mama (Mom)
    "рука", // ruka (Hand)
    "нога", // noga (Leg)
    "мост", // most (Bridge)
    "тест", // test (Test)
    "дверь", // dver' (Door)
    "парк", // park (Park)
  ],

  //ukraine words

  Ukrainian: [
    // Longer words (6 to 8 characters)
    "комп'ютер", // komputer (Computer)
    "освіта", // osvita (Education)
    "економіка", // ekonomika (Economics)
    "психологія", // psykholohiya (Psychology)
    "фінанси", // finansy (Finance)
    "технологія", // tekhnolohiya (Technology)
    "наука", // nauka (Science)
    "розвиток", // rozvytok (Devel

    // Shorter words (3 to 5 characters)
    "мир", // myr (World)
    "будинок", // budynok (House)
    "вода", // voda (Water)
    "книга", // knyha (Book)
    "день", // denʹ (Day)
    "мама", // mama (Mom)
    "тест", // test (Test)
    "парк", // park (Park)
    "місто", // misto (City)
    "сік", // sik (Juice)
    "сніг", // snig (Snow)
    "літо", // lito (Summer)
    "жовтень", // zhovtenʹ (October)
    "собака", // sobaka (Dog)
    "кіт", // kit (Cat)
    "птиця", // ptitsya (Bird)
    "молоко", // moloko (Milk)
    "місце", // mistse (Place)
    "рука", // ruka (Hand)
    "ніж", // nizh (Knife)
  ],

  //thai

  Thai: [
    // Longer words (6 to 8 characters)
    "คอมพิวเตอร์", // Computer (Komphiyutē)
    "การศึกษา", // Education (Kān Sưkṣā)
    "เศรษฐศาสตร์", // Economics (Sētthāsāt)
    "จิตวิทยา", // Psychology (Jit Witthayā)
    "การเงิน", // Finance (Kān Ngoen)
    "เทคโนโลยี", // Technology (Theknolōyī)
    "องค์กร", // Organization (Ongkōn)
    "การวิจัย", // Research (Kān Wijāi)
    "การพัฒนา", // Development (Kān Phatthanā)
    "อินเทอร์เน็ต", // Internet (Inthœ̄nēt)
    "การเผยแพร่", // Publication (Kān Phēiphraē)
    "คำถาม", // Question (Khamthām)
    "ธุรกิจ", // Enterprise (Thurakit)
    "ความต้องการ", // Needs (Khwām Tǭngkān)
    "สัญญา", // Contract (Sānyā)

    // Shorter words (3 to 5 characters)
    "แมว", // Cat (Maew)
    "สุนัข", // Dog (Sunak)
    "หนังสือ", // Book (Nangsu)
    "น้ำ", // Water (Nām)
    "บ้าน", // House (Bān)
    "เมือง", // City (Mūang)
    "อาหาร", // Food (Āhān)
    "หลอด", // Lamp (Lòt)
    "โต๊ะ", // Table (Tō)
    "ประตู", // Door (Pratū)
    "ต้นไม้", // Tree (Tǭn Mái)
    "ปลา", // Fish (Plā)
    "มือ", // Hand (Mūa)
    "ถนน", // Street (Thanon)
    "เซน", // Sun (Sén)
    "ท้องฟ้า", // Sky (Thōngfā)
    "รถ", // Car (Rót)
    "รองเท้า", // Shoe (Rōngthāo)
    "จักรยาน", // Bike (Jakkāyān)
    "นม", // Milk (Nom)
  ],

  Turkish: [
    // Longer words (6 to 8 characters)
    "eğitim", // Education
    "ekonomi", // Economics
    "psikoloji", // Psychology
    "finans", // Finance
    "teknoloji", // Technology
    "bilgi", // Information
    "toplum", // Society
    "pratik", // Practice
    "proje", // Project
    "internet", // Internet
    "yayın", // Publication
    "soru", // Question
    "şirket", // Enterprise
    "ihtiyaç", // Needs
    "sözleşme", // Contract
    "belge", // Document
    "reform", // Reform
    "reklam", // Advertising
    "taşıma", // Transport
    "uzman", // Specialist
    "sistem", // System
    "değişim", // Changes
    "deneyim", // Experience
    "planlama", // Planning
    "strateji", // Strategy
    "üretim", // Production

    // Shorter words (3 to 5 characters)
    "kedi", // Cat
    "köpek", // Dog
    "kitap", // Book
    "su", // Water
    "ev", // House
    "şehir", // City
    "yemek", // Food
    "lamba", // Lamp
    "masa", // Table
    "kapı", // Door
    "ağaç", // Tree
    "balık", // Fish
    "el", // Hand
    "sokak", // Street
    "güneş", // Sun
    "gökyüzü", // Sky
    "araba", // Car
    "ayakkabı", // Shoe
    "bisiklet", // Bike
    "süt", // Milk
  ],

  German: [
    // Longer words (6 to 8 characters)
    "Computer", // Computer
    "Bildung", // Education
    "Ökonomie", // Economics
    "Psychologie", // Psychology
    "Finanzen", // Finance
    "Bedürfnisse", // Needs
    "Vertrag", // Contract
    "Dokument", // Document
    "Reform", // Reform
    "Werbung", // Advertising
    "Transport", // Transport
    "Spezialist", // Specialist
    "System", // System
    "Änderungen", // Changes

    // Shorter words (3 to 5 characters)
    "Katze", // Cat
    "Hund", // Dog
    "Buch", // Book
    "Wasser", // Water
    "Haus", // House
    "Stadt", // City
    "Essen", // Food
    "Lampe", // Lamp
    "Tisch", // Table
    "Tür", // Door
    "Baum", // Tree
    "Fisch", // Fish
    "Hand", // Hand
    "Straße", // Street
    "Sonne", // Sun
    "Himmel", // Sky
    "Auto", // Car
    "Schuh", // Shoe
    "Fahrrad", // Bike
    "Milch", // Milk
  ],

  //start pashto
  Pashto: [
    // Longer words (6 to 8 characters)
    "تعلیم", // Education (Taʿlīm)
    "اقتصاد", // Economics (Iqtisād)
    "مالیات", // Finance (Māliyāt)
    "پوهنتون", // University (Puhantūn)
    "پوښتنه", // Question (Pūshtanah)
    "شرکت", // Enterprise (Shirkāt)
    "اړتیاوې", // Needs (Arṭiyāwē)
    "قرارداد", // Contract (Qārārdād)
    "سند", // Document (Sand)
    "اصلاح", // Reform (Islāḥ)
    "اعلان", // Advertising (Aʿlān)
    "متخصص", // Specialist (Mutakhassis)
    "سیستم", // System (Sīstəm)
    "تغییرات", // Changes (Taghīrāt)
    "تیاری", // Preparation (Tīārī)
    "فعالیت", // Activity (Faʿāliyat)
    "شراکت", // Partnership (Shirākat)
    "تجربه", // Experience (Tajriba)
    "پلان", // Planning (Plan)
    "تولید", // Production (Tawlīd)

    // Shorter words (3 to 5 characters)
    "پیشو", // Cat (Pēsho)
    "سپي", // Dog (Spē)
    "کتاب", // Book (Kitāb)
    "اوبه", // Water (Ōbah)
    "کور", // House (Kōr)
    "ښار", // City (Shār)
    "خوراک", // Food (Khurāk)
    "بجلی", // Lamp (Bijlī)
    "میز", // Table (Miz)
    "دروازه", // Door (Darwāzah)
    "درخت", // Tree (Darakht)
    "ماهی", // Fish (Māhī)
    "لاس", // Hand (Lās)
    "کوڅه", // Street (Kūchah)
    "لمر", // Sun (Lamar)
    "اسمان", // Sky (Asman)
    "موټر", // Car (Mōṭar)
    "بوټ", // Shoe (Bōṭ)
  ],

  //portuguese
  Portuguese: [
    "computador", // Computer
    "educação", // Education
    "economia", // Economicsy
    "informação", // Information
    "sociedade", // Society
    "pratica", // Practice
    "projeto", // Project
    "rede", // Internet
    "publicação", // Publication
    "questão", // Question
    "emprego", // Enterprise
    "necessidade", // Needs
    "contrato", // Contract
    "documento", // Document
    "reforma", // Reform
    "producção", // Production
  ],

  //japanese
  Japanese: [
    "コンピューター", // Computer
    "教育", // Education
    "経済", // Economics
    "認識", // Recognition
    "財政", // Finance
    "技術", // Technology
    "組織", // Organization
    "研究", // Research
    "開発", // Development
    "大学", // University
    "情報", // Information
    "協会", // Society
    "練習", // Practice
    "プロジェクト", // Project
    "インターネット", // Internet
    "公開", // Publication
    "質問", // Question
    "企業", // Enterprise
    "ニーズ", // Needs
    "契約", // Contract
    "書類", // Document
    "改正", // Reform
    "広告", // Advertising
    "輸送", // Transport
    "専門家", // Specialist
    "システム", // System
    "変更", // Changes
    "準備", // Preparation
    "活動", // Activity
    "協力", // Collaboration
    "経験", // Experience
    "企画", // Planning
    "戦略", // Strategy
    "生産", // Production
  ],
  Abkhaz: [
    // Shorter words (3 to 5 characters)
    "аҟ", // Water
    "аба", // Father
    "ама", // Mother
    "ахы", // Head
    "уа", // You
    "аи", // Yes
    "ҭаа", // Go
    "ахра", // Home
    "бжьы", // Voice
    "уаҩ", // Person
    "нап", // Hand
    "мыц", // Boat
    "кьы", // Night
    "аҭ", // Bird
    "лых", // Eye
    "ҿыр", // Fire
    "зны", // Again
    "ҩба", // Two
    "хы", // Head
    "гәы", // Heart
    "ԥа", // Boy
    "ара", // Here
    "ус", // Work

    // Longer words (6 to 8 characters)
    "аамҭа", // Time
    "дырра", // Knowledge
    "алшара", // Power
    "ахәаԥш", // Watch
    "аамҭа", // Time
    "амилаҭ", // Nation
    "ачҳара", // Face
    "ашкол", // School
    "аԥсабара", // Nature
    "адырра", // Information
    "аҭакра", // Answer
    "агәра", // Belief
    "аԥсы", // Soul
    "апсуа", // Abkhazian (person)
    "аҩны", // House
    "амшын", // Sea
    "арацәа", // Stars
    "ажәа", // Word
    "ахәа", // Animal
    "агьа", // Wind
    "аҭах", // Story
  ],

  Afrikaans: [
    // Shorter words (3 to 5 characters)
    "son", // Sun
    "see", // Sea
    "bok", // Goat
    "kat", // Cat
    "vis", // Fish
    "kop", // Head
    "oog", // Eye
    "pad", // Road
    "boek", // Book
    "huis", // House
    "wind", // Wind
    "dorp", // Town
    "vuur", // Fire
    "kind", // Child
    "geld", // Money
    "boom", // Tree
    "veld", // Field
    "lug", // Air
    "ster", // Star
    "ys", // Ice
    "kos", // Food
    "man", // Man
    "vrou", // Woman
    "hond", // Dog
    "kar", // Car
    "muur", // Wall
    "klip", // Stone
    "lys", // List
    "reën", // Rain
    "brug", // Bridge

    // Longer words (6 to 8 characters)
    "kinders", // Children
    "gesin", // Family
    "skool", // School
    "gesonde", // Healthy
    "rekenaar", // Computer
    "vriende", // Friends
    "universiteit", // University
    "landbou", // Agriculture
    "gesondheid", // Health
    "vakansie", // Holiday
    "produkte", // Products
    "natuur", // Nature
    "samelewing", // Society
    "diere", // Animals
    "vliegtuig", // Airplane
    "onderwys", // Education
    "bestuur", // Management
    "lewe", // Life
    "wêreld", // World
    "bevolking", // Population
    "musiek", // Music
    "vervoer", // Transport
    "kultuur", // Culture
  ],

  Vietnamese: [
    // Shorter words (3 to 5 characters)
    "mưa", // Rain
    "nước", // Water
    "cây", // Tree
    "bàn", // Table
    "mắt", // Eye
    "tai", // Ear
    "cá", // Fish
    "trời", // Sky
    "xe", // Car
    "mẹ", // Mother
    "bố", // Father
    "bé", // Child
    "nhà", // House
    "đất", // Land
    "cửa", // Door
    "đèn", // Lamp
    "sách", // Book
    "bút", // Pen
    "trẻ", // Young
    "đường", // Road
    "hoa", // Flower
    "mặt", // Face
    "tóc", // Hair
    "môi", // Lips
    "áo", // Shirt
    "tay", // Hand
    "bạn", // Friend
    "chó", // Dog
    "mèo", // Cat
    "gió", // Wind
    "đá", // Stone

    // Longer words (6 to 8 characters)
    "máy tính", // Computer
    "người bạn", // Friend
    "ngôi nhà", // House
    "giáo dục", // Education
    "sinh viên", // Student
    "bệnh viện", // Hospital
    "thực phẩm", // Food
    "trường học", // School
    "gia đình", // Family
    "thành phố", // City
    "nông nghiệp", // Agriculture
    "khoa học", // Science
    "lịch sử", // History
    "thiên nhiên", // Nature
    "cộng đồng", // Community
    "phát triển", // Development
    "giao thông", // Transport
    "sức khỏe", // Health
    "âm nhạc", // Music
    "nghệ thuật", // Art
    "văn hóa", // Culture
    "thế giới", // World
    "chính phủ", // Government
    "công nghệ", // Technology
  ],

  Tamil: [
    // Shorter words (3 to 5 characters)
    "நீர்", // Water
    "மண்", // Earth
    "நாய்", // Dog
    "மரம்", // Tree
    "கை", // Hand
    "பூ", // Flower
    "நகம்", // Nail
    "கல்", // Stone
    "வீடு", // House
    "பால்", // Milk
    "சோறு", // Rice
    "மழை", // Rain
    "நிலை", // Land
    "வெயில்", // Sun
    "புத்தகம்", // Book
    "கார்", // Car
    "வசந்தம்", // Spring
    "அம்மா", // Mother
    "அப்பா", // Father
    "தோழி", // Friend
    "விழி", // Eye
    "காது", // Ear
    "முயல்", // Rabbit
    "தம்பி", // Younger brother
    "படகு", // Boat
    "மண்", // Soil
    "மனிதன்", // Man
    "நெருப்பு", // Fire
    "சுவர்", // Wall
    "நகரம்", // City

    // Longer words (6 to 8 characters)
    "கணினி", // Computer
    "வளர்ச்சி", // Development
    "நாடகம்", // Drama
    "விண்வெளி", // Space
    "விடுமுறை", // Holiday
    "மாணவர்", // Student
    "பயிற்சி", // Training
    "மருத்துவமனை", // Hospital
    "மொழி", // Language
    "கற்பிதம்", // Education
    "நண்பர்கள்", // Friends
    "சுற்றுச்சூழல்", // Environment
    "அரசு", // Government
    "கலை", // Art
    "இசை", // Music
    "நோயாளி", // Patient
    "அறிவியல்", // Science
    "உலகம்", // World
    "சமூகம்", // Society
    "சுகாதாரம்", // Health
    "தொழில்நுட்பம்", // Technology
    "வாணிபம்", // Commerce
    "வணிகம்", // Business
    "விளையாட்டு", // Sports
    "இயற்கை", // Nature
  ],
  Telugu: [
    // Shorter words (3 to 5 characters)
    "నీరు", // Water
    "నిహారిక", // Sky
    "చెట్టు", // Tree
    "పిల్ల", // Child
    "పూలు", // Flowers
    "కలాలు", // Dreams
    "కళ్లు", // Eyes
    "రాయి", // Stone
    "పది", // Ten
    "కాఫీ", // Coffee
    "పాలు", // Milk
    "వాన", // Rain
    "మాట", // Word
    "స్నేహం", // Friendship
    "పుస్తకం", // Book
    "తల్లి", // Mother
    "నాన్న", // Father
    "ఆకాశం", // Sky
    "తీరం", // Shore
    "నది", // River
    "చిలుక", // Parrot
    "చెప్పు", // Shoe
    "గాలి", // Wind
    "నిమ్మ", // Lemon
    "పండు", // Fruit
    "పాప", // Baby Girl
    "వీధి", // Street
    "సిరి", // Wealth
    "అమ్మ", // Mom
    "నివాసం", // Home

    // Longer words (6 to 8 characters)
    "కంప్యూటర్", // Computer
    "విద్యార్థి", // Student
    "విశ్వవిద్యాలయం", // University
    "విద్య", // Education
    "ఆరోగ్యం", // Health
    "వైద్యం", // Medicine
    "సముద్రం", // Ocean
    "ప్రగతి", // Progress
    "సంస్కృతి", // Culture
    "రాజకీయాలు", // Politics
    "విహారం", // Tourism
    "పరిశోధన", // Research
    "పర్యావరణం", // Environment
    "పరిశ్రమ", // Industry
    "వ్యవసాయం", // Agriculture
    "సమాజం", // Society
    "ఆర్థికం", // Economics
    "న్యూస్", // News
    "సంఘం", // Community
    "కుటుంబం", // Family
    "వ్యాపారం", // Business
    "సేవలు", // Services
    "సాహిత్యం", // Literature
    "సంస్కరణలు", // Reforms
    "పరిశీలన", // Analysis
    "పరీక్ష", // Exam
  ],
  Manipuri: [
    // Shorter words (3 to 5 characters)
    "ꯄꯥꯌꯆꯥ", // Water
    "ꯃꯂꯥ", // House
    "ꯁꯃꯄꯤ", // Child
    "ꯃꯇꯝ", // Mother
    "ꯅꯨꯝ", // Father
    "ꯇꯅꯤꯡ", // Height
    "ꯄꯥꯛ", // Book
    "ꯀꯨꯑꯣ", // Dog
    "ꯂꯤꯛ", // Wax
    "ꯄꯅꯥ", // Moon
    "ꯅꯢꯟ", // Sun
    "ꯃꯅꯥ", // Rain
    "ꯈꯔꯤ", // Idol
    "ꯃꯅ", // Tree
    "ꯃꯨꯟ", // Talk
    "ꯈꯪꯂꯤ", // Apple
    "ꯀꯄꯥ", // Chili
    "ꯉꯅꯥ", // River
    "ꯁꯨꯛ", // Road
    "ꯃꯁꯤꯅ", // Daily
    "ꯊꯨꯝ", // Music
    "ꯂꯅꯟ", // Seed
    "ꯃꯔꯤ", // Soil
    "ꯃꯤꯡ", // Word
    "ꯃꯉ", // Black
    "ꯃꯩ", // White
    "ꯉꯕ", // Flower
    "ꯃꯣꯛ", // Boy
    "ꯃꯤ", // Hard
    "ꯈꯕ", // Outside

    // Longer words (6 to 8 characters)
    "ꯁꯤꯛꯄ", // Education
    "ꯉꯄꯥ", // Student
    "ꯃꯇ", // Culture
    "ꯈꯂꯥ", // Success
    "ꯁꯥ", // Farming
    "ꯇꯅ", // Social
    "ꯁꯤꯛ", // Science
    "ꯉꯟ", // Organization
    "ꯃꯅꯥ", // Family
    "ꯇꯅꯥ", // Cultural
    "ꯂꯥꯂꯤ", // Health
    "ꯄꯥ", // Politics
    "ꯇꯅꯥ", // Meeting
    "ꯄꯥꯔꯥ", // Goodness
    "ꯋꯠꯅ", // Technical
    "ꯃꯊꯥ", // Morality
    "ꯁꯥꯂꯤ", // Sociability
    "ꯆꯪꯂꯥ", // Love
    "ꯌꯤ", // Security
    "ꯇꯅ", // Knowledge
    "ꯀꯨ", // Games
    "ꯇꯅꯥ", // Institution
    "ꯄꯅ", // Potential
    "ꯄꯥꯔꯥ", // Behavior
    "ꯆꯝꯂ", // Discussion
  ],

  Malayalam: [
    // Shorter words (3 to 5 characters)
    "വീട്", // House
    "ജലം", // Water
    "പൂവ്", // Flower
    "മരം", // Tree
    "കൈ", // Hand
    "വീട്", // Home
    "പൂ", // Flower
    "കാക്ക", // Crow
    "പൂച്ച", // Cat
    "ആകാശം", // Sky
    "മഴ", // Rain
    "പണ്ട്", // Fruit
    "പനി", // Fever
    "പാത", // Road
    "ചൂട്", // Heat
    "വീട്", // House
    "മാവ്", // Mango tree
    "ചെപ്പ്", // Shoe
    "ചിത്രം", // Picture
    "കുറി", // Mark
    "കത്ത്", // Letter
    "നദി", // River
    "മണല്", // Sand
    "കടല്", // Sea
    "പുഴ", // River
    "കാട്", // Forest
    "മണ്", // Soil
    "വാനം", // Sky
    "തനി", // Pure
    "സാധ", // Simple

    // Longer words (6 to 8 characters)
    "കമ്പ്യൂട്ടർ", // Computer
    "വിദ്യാർത്ഥി", // Student
    "ആശുപത്രി", // Hospital
    "പ്രവർത്തനം", // Work
    "വായനശാല", // Library
    "സമൂഹം", // Society
    "സാഹിത്യം", // Literature
    "വ്യാപാരം", // Business
    "പഠനം", // Study
    "ശാസ്ത്രം", // Science
    "വിദ്യാലയം", // School
    "കൃഷി", // Agriculture
    "പ്രകൃതി", // Nature
    "സഞ്ചാരം", // Travel
    "ആരോഗ്യം", // Health
    "പരിസ്ഥിതി", // Environment
    "സിനിമ", // Cinema
    "സമൂഹം", // Community
    "കവിത", // Poem
    "കല", // Art
    "സമരം", // Strike
    "സന്തോഷം", // Happiness
    "സംഘടന", // Organization
    "ഭാരതം", // India
    "ചരിത്രം", // History
  ],

  Kashmiri: [
    // Shorter words (3 to 5 characters)
    "پانی", // Water
    "درخت", // Tree
    "گھر", // House
    "گاو", // Cow
    "آسمان", // Sky
    "پتھ", // Stone
    "پھل", // Fruit
    "کتاب", // Book
    "ہوا", // Wind
    "دوست", // Friend
    "آگ", // Fire
    "پھیر", // Cloud
    "راستہ", // Road
    "سرد", // Cold
    "چای", // Tea
    "پھول", // Flower
    "چاند", // Moon
    "سورج", // Sun
    "دریا", // River
    "باد", // Air
    "رات", // Night
    "کھیل", // Game
    "نمک", // Salt
    "ہاتھ", // Hand
    "نیل", // Blue
    "جل", // Burn
    "مٹی", // Soil
    "نم", // Wet
    "بال", // Hair
    "بیٹی", // Daughter

    // Longer words (6 to 8 characters)
    "تعلیم", // Education
    "کامیابی", // Success
    "کتھوالی", // Storytelling
    "پریوار", // Family
    "مہربان", // Kindness
    "دستکار", // Artisan
    "اداکار", // Actor
    "دوستی", // Friendship
    "صحت", // Health
    "معاشرت", // Society
    "معلومات", // Information
    "ترقی", // Development
    "زبان", // Language
    "سفر", // Journey
    "ثقافت", // Culture
    "تنظیم", // Organization
    "فلسفہ", // Philosophy
    "سائنس", // Science
    "حکومت", // Government
    "محبت", // Love
    "کشمیری", // Kashmiri
    "معاشرتی", // Social
    "دوستی", // Friendship
    "اقتصاد", // Economy
    "موسیقی", // Music
  ],

  Gujarati: [
    // Shorter words (3 to 5 characters)
    "પાણી", // Water
    "બાળક", // Child
    "ઘર", // House
    "કૂતરો", // Dog
    "પુસ્તક", // Book
    "સૂરજ", // Sun
    "ચાંદ", // Moon
    "ફૂલો", // Flowers
    "પંખી", // Bird
    "લંબો", // Tall
    "સ્નેહ", // Love
    "સફેદ", // White
    "ભૂત", // Ghost
    "રસ્તો", // Road
    "ઘડી", // Clock
    "શાળા", // School
    "ભાષા", // Language
    "ખીચો", // Pull
    "જાતો", // Cast
    "વૃક્ષ", // Tree
    "ઝાડ", // Branch
    "ખેતર", // Farm
    "ખોરાક", // Food
    "દર્દ", // Pain
    "તડકું", // Sunlight
    "સમંદર", // Ocean
    "ગરમી", // Heat
    "સમય", // Time
    "તરસ", // Thirst

    // Longer words (6 to 8 characters)
    "શિક્ષણ", // Education
    "વિદ્યાર્થી", // Student
    "વિશ્વવિદ્યાલય", // University
    "વિકાસ", // Development
    "આરોગ્ય", // Health
    "સમાજ", // Society
    "સાંસ્કૃતિક", // Cultural
    "પ્રેમભાષા", // Love Language
    "વાણિજ્ય", // Commerce
    "કાળગતિ", // Time Flow
    "સંચાર", // Communication
    "પરિવર્તન", // Change
    "સંસ્થાન", // Organization
    "શાંતિ", // Peace
    "વિશ્વાસ", // Trust
    "કથાવિદ", // Storyteller
    "સંભવનાઓ", // Opportunities
    "યોજનાઓ", // Plans
    "જ્ઞાન", // Knowledge
    "પરિપ્રેક્ષ્ય", // Perspective
    "અર્થશાસ્ત્ર", // Economics
    "વિજ્ઞાન", // Science
    "સૃષ્ટિ", // Creation
    "સંગ્રહ", // Collection
    "શ્રેષ્ઠતા", // Excellence
    "સેવા", // Service
  ],

  Punjabi: [
    // Shorter words (3 to 5 characters)
    "ਪਾਣੀ", // Water
    "ਘਰ", // House
    "ਬਾਲਕ", // Child
    "ਮਾਂ", // Mother
    "ਬਾਪ", // Father
    "ਕੱਦ", // Height
    "ਕਿਤਾਬ", // Book
    "ਕੌਲਾ", // Dog
    "ਮੋਮ", // Wax
    "ਚੰਦ", // Moon
    "ਸੂਰਜ", // Sun
    "ਮੀਂਹ", // Rain
    "ਬੁੱਤ", // Idol
    "ਦਰੱਖਤ", // Tree
    "ਬਾਤ", // Talk
    "ਸੇਬ", // Apple
    "ਮਿਰਚ", // Chili
    "ਨਦੀਆਂ", // Rivers
    "ਸੜਕ", // Road
    "ਨਿੱਤ", // Daily
    "ਬਾਜਾ", // Music
    "ਬੀਜ", // Seed
    "ਮਿੱਟੀ", // Soil
    "ਗੱਲ", // Word
    "ਸਿਆਹ", // Black
    "ਸਫੈਦ", // White
    "ਫੁੱਲ", // Flower
    "ਮੁੰਡਾ", // Boy
    "ਕੜਕ", // Hard
    "ਬਾਹਰ", // Outside

    // Longer words (6 to 8 characters)
    "ਸਿੱਖਿਆ", // Education
    "ਵਿਦਿਆਰਥੀ", // Student
    "ਸੰਸਕਾਰ", // Culture
    "ਕਾਮਯਾਬੀ", // Success
    "ਕਿਸਾਨੀ", // Farming
    "ਸਮਾਜਿਕ", // Social
    "ਵਿਜ್ಞಾನ", // Science
    "ਸੰਗਠਨ", // Organization
    "ਪਰਿਵਾਰ", // Family
    "ਸਾਂਸਕ੍ਰਿਤਿਕ", // Cultural
    "ਸਿਹਤ", // Health
    "ਰਾਜਨੀਤੀ", // Politics
    "ਮੁਲਾਕਾਤ", // Meeting
    "ਚੰਗਾਈ", // Goodness
    "ਤਕਨੀਕੀ", // Technical
    "ਨੈਤਿਕਤਾ", // Morality
    "ਸਮਾਜਿਕਤਾ", // Sociability
    "ਮੁਹੱਬਤ", // Love
    "ਸੁਰੱਖਿਆ", // Security
    "ਵਿਦਿਆ", // Knowledge
    "ਖੇਡਾਂ", // Games
    "ਸੰਸਥਾ", // Institution
    "ਸੰਭਾਵਨਾ", // Potential
    "ਵਿਵਹਾਰ", // Behavior
    "ਗੱਲਬਾਤ", // Discussion
  ],

  Sanskrit: [
    // Shorter words (3 to 5 characters)
    "जल", // Water
    "गृह", // House
    "बालक", // Child
    "माता", // Mother
    "पिता", // Father
    "पुष्प", // Flower
    "वृक्ष", // Tree
    "गाय", // Cow
    "सूर्य", // Sun
    "चन्द्र", // Moon
    "नदी", // River
    "पथ", // Path
    "वायु", // Air
    "पञ्च", // Five
    "अग्नि", // Fire
    "कवि", // Poet
    "धन", // Wealth
    "दिवस", // Day
    "रात्रि", // Night
    "सखा", // Friend
    "शब्द", // Word
    "तारक", // Star
    "धर्म", // Duty
    "शान्ति", // Peace
    "राशि", // Amount
    "कल्याण", // Welfare
    "धृति", // Patience
    "कला", // Art
    "विचार", // Thought
    "मेष", // Aries

    // Longer words (6 to 8 characters)
    "विद्या", // Knowledge
    "शिक्षा", // Education
    "विश्वविद्यालय", // University
    "समाज", // Society
    "कृषि", // Agriculture
    "स्वास्थ्य", // Health
    "संस्कृति", // Culture
    "धर्मशास्त्र", // Religious Text
    "संपत्ति", // Property
    "सुखदायी", // Pleasant
    "परिवर्तन", // Change
    "संदेश", // Message
    "शान्तिका", // Peacefulness
    "संस्कृत", // Refined
    "आवश्यकता", // Necessity
    "प्रतिभा", // Talent
    "समृद्धि", // Prosperity
    "परिवार", // Family
    "प्रसन्नता", // Happiness
    "विश्वास", // Trust
    "उपकार", // Kindness
    "व्यवस्था", // Arrangement
    "सङ्गीत", // Music
    "योग्यता", // Qualification
    "उपदेश", // Instruction
  ],

  Sindhi: [
    // Shorter words (3 to 5 characters)
    "پاني", // Water
    "گهر", // House
    "بچو", // Child
    "ماڻھو", // Person
    "ماءُ", // Mother
    "بابا", // Father
    "چنڊ", // Moon
    "سورج", // Sun
    "پھول", // Flower
    "گلاب", // Rose
    "ڪتا", // Dog
    "وڻ", // Tree
    "رستو", // Path
    "نهر", // Canal
    "شمع", // Candle
    "گھڻو", // Much
    "پڻ", // Also
    "سالن", // Year
    "باجي", // Elder sister
    "دوست", // Friend
    "ڌرتي", // Earth
    "غزل", // Ghazal
    "نغمه", // Melody
    "بندھ", // Tie
    "چنر", // Moonlight
    "رنگ", // Color
    "چوڪ", // Square
    "چمچو", // Spoon
    "دودھ", // Milk
    "پتنگ", // Kite

    // Longer words (6 to 8 characters)
    "تعليم", // Education
    "تعليمات", // Instructions
    "سماج", // Society
    "معلومات", // Information
    "سندھيوار", // Sindhi language
    "اجتماع", // Gathering
    "دوستاڻو", // Friendship
    "ايمان", // Faith
    "سخاوت", // Generosity
    "خوشي", // Happiness
    "صحت", // Health
    "صديق", // Truthful
    "وڏيءَ", // Magnificent
    "آزادي", // Freedom
    "نظامت", // Administration
    "سماجي", // Social
    "ثقافتي", // Cultural
    "بازار", // Market
    "خوابن", // Dreams
    "خوشبو", // Fragrance
    "روشنائي", // Enlightenment
    "درد", // Pain
    "شعراء", // Poets
    "فلسفہ", // Philosophy
    "خوبصورتي", // Beauty
    "دلچسپی", // Interest
    "موتيا", // Jasmine
  ],
  Oriya: [
    // Shorter words (3 to 5 characters)
    "ପାଣି", // Water
    "ଗୃହ", // House
    "ବାଳକ", // Child
    "ମା", // Mother
    "ବାପା", // Father
    "କୁତ୍ତା", // Dog
    "ଗଛ", // Tree
    "ମାଛ", // Fish
    "ପୁସ୍ତକ", // Book
    "ଗୋଡ", // Leg
    "ଚନ୍ଦ୍ର", // Moon
    "ସୂର୍ୟ", // Sun
    "ପଥ", // Path
    "ଜଳ", // Liquid
    "ସେବ", // Apple
    "ପ୍ରେମ", // Love
    "କଥା", // Word
    "ରାତି", // Night
    "ସଂଗୀତ", // Music
    "ବର୍ଷ", // Year
    "ଶିଶୁ", // Infant
    "ଭୋଜନ", // Food
    "ଚିତ୍ର", // Picture
    "ଗାୟ", // Cow
    "ଗତି", // Motion
    "ଦୀପ", // Lamp
    "ଚିଠି", // Letter
    "କଳା", // Art
    "ସମୟ", // Time
    "ତଳ", // Bottom

    // Longer words (6 to 8 characters)
    "ଶିକ୍ଷା", // Education
    "ବିଦ୍ୟାର୍ଥୀ", // Student
    "କୃଷି", // Agriculture
    "ସଂସ୍କୃତି", // Culture
    "ସମାଜ", // Society
    "ସ୍ୱାସ୍ଥ୍ୟ", // Health
    "ବିଶ୍ୱବିଦ୍ୟାଳୟ", // University
    "ଶାନ୍ତି", // Peace
    "ପରିବାର", // Family
    "ଆଧୁନିକ", // Modern
    "ସୃଷ୍ଟି", // Creation
    "ଆନନ୍ଦ", // Joy
    "ନିୟମ", // Rule
    "ପରିବର୍ତ୍ତନ", // Change
    "ବିଶ୍ୱସ", // Trust
    "ବିଜ୍ଞାନ", // Science
    "ବିକାସ", // Development
    "ଜ୍ଞାନ", // Knowledge
    "ପରିବେଷ", // Environment
    "ପ୍ରକୃତି", // Nature
    "ସମୃଦ୍ଧି", // Prosperity
    "ବାଣିଜ୍ୟ", // Commerce
    "ଗୁଣବତ୍ତା", // Quality
    "ସଙ୍ଗୀତ", // Music
    "ଦୟା", // Compassion
    "କୃତି", // Creation
    "ଶାସନ", // Governance
  ],
  Kannada: [
    // Shorter words (3 to 5 characters)
    "ನೀರು", // Water
    "ಮನೆ", // House
    "ಮಕ್ಕಳ", // Children
    "ತಾಯಿ", // Mother
    "ತಂದೆ", // Father
    "ಕುಕ್ಕರು", // Dog
    "ಮರ", // Tree
    "ಮೀನ", // Fish
    "ಪುಸ್ತಕ", // Book
    "ಚಂದ್ರ", // Moon
    "ಸೂರ್ಯ", // Sun
    "ದಾರಿ", // Path
    "ಜಲ", // Liquid
    "ಸೇಬು", // Apple
    "ಪ್ರೀತಿ", // Love
    "ಬಾತ್", // Talk
    "ರಾತ್ರಿ", // Night
    "ಮೂಡು", // Cloud
    "ಬೆಳಕು", // Light
    "ಬಾಯ", // Mouth
    "ಹಕ್ಕಿ", // Bird
    "ನಗು", // Smile
    "ಆಕಾಶ", // Sky
    "ಕೋಣ", // Angle
    "ಗಿಡ", // Plant
    "ಮಂಜು", // Mist
    "ಕಾಳ", // Dark
    "ಚುಟುಕು", // Short
    "ಶಿಕ್ಷ", // Teacher

    // Longer words (6 to 8 characters)
    "ಶಿಕ್ಷಣ", // Education
    "ವಿದ್ಯಾರ್ಥಿ", // Student
    "ಕೃಷಿ", // Agriculture
    "ಸಂಸ್ಕೃತಿ", // Culture
    "ಸಮಾಜ", // Society
    "ಆರೋಗ್ಯ", // Health
    "ವಿಶ್ವವಿದ್ಯಾಲಯ", // University
    "ಶಾಂತಿ", // Peace
    "ಪರಿವಾರ", // Family
    "ಆಧುನಿಕ", // Modern
    "ಸೃಷ್ಟಿ", // Creation
    "ಆನಂದ", // Joy
    "ನಿಯಮ", // Rule
    "ಪರಿವರ್ತನೆ", // Change
    "ನಂಬಿಕೆ", // Trust
    "ವಿಜ್ಞಾನ", // Science
    "ವಿಕಾಸ", // Development
    "ಜ್ಞಾನ", // Knowledge
    "ಪರಿವೇಶ", // Environment
    "ಪ್ರಕೃತಿ", // Nature
    "ಶ್ರೇಣೀಬದ್ಧ", // Organized
    "ಸಮೃದ್ಧಿ", // Prosperity
    "ಬಾಣಿಜ್ಯ", // Commerce
    "ಗुणಮಟ್ಟ", // Quality
    "ಸಂಗೀತ", // Music
    "ದಯಾ", // Compassion
    "ಅಭ್ಯಾಸ", // Practice
    "ಭಾವನೆ", // Emotion
  ],

  Chakma: [
    // Shorter words (3 to 5 characters)
    "𑄛𑄮𑄢𑄴𑄢𑄴", // Wax
    "𑄝𑄝𑄨𑄢𑄴𑄢𑄴", // Moon
    "𑄝𑄇𑄧𑄢𑄴𑄢𑄴", // Sun
    "𑄝𑄝𑄢𑄧𑄢𑄴𑄢𑄴", // Rain
    "𑄢𑄪𑄥𑄧𑄢𑄴𑄢𑄴", // Idol
    "𑄛𑄮𑄢𑄴", // Tree
    "𑄛𑄮𑄟𑄴", // River
    "𑄝𑄧𑄇𑄢𑄴", // Fish
    "𑄖𑄪𑄑𑄴𑄓𑄴", // Book
    "𑄣𑄪𑄌𑄢𑄴", // Light
    "𑄉𑄴𑄢𑄴", // Heart
    "𑄜𑄧𑄢𑄴", // Earth
    "𑄚𑄧𑄬𑄚𑄴", // Star
    "𑄚𑄧𑄖𑄴", // Name
    "𑄟𑄧𑄢𑄴", // Child
    "𑄝𑄧𑄇𑄴", // Sky
    "𑄢𑄮𑄚𑄴", // Hand
    "𑄉𑄦𑄊𑄴", // Peace
    "𑄖𑄨𑄢𑄴", // Milk
    "𑄝𑄟𑄬𑄢𑄴", // Flower
    "𑄃𑄌𑄴", // Fire
    "𑄝𑄧𑄟𑄴", // Wind
    "𑄢𑄧𑄖𑄴", // Color
    "𑄠𑄬𑄇𑄴", // Salt
    "𑄙𑄦𑄙𑄴", // Road
    "𑄚𑄖𑄴", // King
    "𑄟𑄨𑄬𑄴", // Young
    "𑄖𑄪𑄖𑄴", // Sweet
    "𑄟𑄖𑄧𑄴", // Joy
    "𑄇𑄬𑄟𑄴", // Home

    // Longer words (6 to 8 characters)
    "𑄙𑄪𑄚𑄧𑄠𑄬", // Education
    "𑄛𑄮𑄟𑄴𑄑𑄴", // Happiness
    "𑄘𑄪𑄧𑄘𑄴𑄘𑄴", // Friendship
    "𑄟𑄧𑄚𑄢𑄴𑄡𑄴", // Family
    "𑄟𑄣𑄴𑄙𑄧𑄢𑄴", // Agriculture
    "𑄑𑄬𑄘𑄴𑄢𑄴𑄢𑄴", // Environment
    "𑄝𑄋𑄖𑄴𑄢𑄴", // Science
    "𑄟𑄧𑄡𑄴𑄢𑄴𑄢𑄴", // Development
    "𑄘𑄢𑄴𑄘𑄧𑄪𑄣𑄴", // Prosperity
    "𑄡𑄨𑄛𑄧𑄟𑄴𑄘𑄴", // Creativity
    "𑄑𑄧𑄘𑄴𑄢𑄧𑄛𑄴", // Tolerance
    "𑄑𑄧𑄓𑄧𑄪𑄧𑄴", // Wisdom
    "𑄟𑄖𑄚𑄴𑄢𑄴", // Relation
    "𑄚𑄖𑄴𑄡𑄴", // Children
    "𑄅𑄋𑄧𑄟𑄴𑄘𑄴", // Opportunity
    "𑄝𑄟𑄢𑄴𑄑𑄧𑄟𑄴", // Communication
    "𑄜𑄖𑄴𑄃𑄬𑄏𑄴", // Satisfaction
    "𑄞𑄨𑄇𑄴𑄦𑄧𑄴", // Technology
    "𑄅𑄖𑄬𑄴𑄚𑄴", // Inspiration
    "𑄞𑄧𑄉𑄨𑄪𑄴", // Kindness
    "𑄞𑄨𑄃𑄨𑄢𑄴", // Motivation
    "𑄛𑄧𑄬𑄴𑄌𑄴", // Tradition
    "𑄅𑄃𑄬𑄬𑄴", // Responsibility
  ],

  Nepali: [
    // Shorter words (3 to 5 characters)
    "पानी", // Water
    "घर", // House
    "बालक", // Child
    "माँ", // Mother
    "बाबा", // Father
    "कुकुर", // Dog
    "रुख", // Tree
    "माछा", // Fish
    "किताब", // Book
    "चाँद", // Moon
    "सूर्य", // Sun
    "मार्ग", // Path
    "आकाश", // Sky
    "सेब", // Apple
    "सच्चा", // True
    "बात", // Word
    "रात", // Night
    "धूप", // Sunshine
    "स्वाद", // Taste
    "हात", // Hand
    "नदी", // River
    "भर", // Full
    "सुख", // Happiness
    "दुख", // Sadness
    "हाँसो", // Smile
    "दृष्टि", // Vision
    "जन्म", // Birth
    "शक्ति", // Power
    "सुन", // Gold
    "मोह", // Desire
    "वायु", // Air

    // Longer words (6 to 8 characters)
    "शिक्षा", // Education
    "सुखद", // Pleasant
    "मित्रता", // Friendship
    "परिवार", // Family
    "कृषि", // Agriculture
    "स्वास्थ्य", // Health
    "विश्वविद्यालय", // University
    "शान्ति", // Peace
    "संस्कृति", // Culture
    "विकास", // Development
    "ज्ञान", // Knowledge
    "पर्यावरण", // Environment
    "मुक्ति", // Freedom
    "आनन्द", // Joy
    "विश्वास", // Trust
    "सामाजिक", // Social
    "संघर्ष", // Struggle
    "प्रगति", // Progress
    "सपना", // Dream
    "शक्ति", // Strength
    "प्रकृति", // Nature
    "सृजनशीलता", // Creativity
    "कर्मठता", // Hard work
    "जिम्मेदारी", // Responsibility
    "सहयोग", // Cooperation
    "सफलता", // Success
    "सङ्गीत", // Music
    "परिस्थिति", // Situation
    "मोहकता", // Attractiveness
    "सकारात्मक", // Positive
    "नकारात्मक", // Negative
  ],

  Dari: [
    // Shorter words (3 to 5 characters)
    "آب", // Water
    "خانه", // House
    "کودک", // Child
    "مادر", // Mother
    "پدر", // Father
    "سگ", // Dog
    "درخت", // Tree
    "ماهی", // Fish
    "کتاب", // Book
    "ماه", // Moon
    "خورشید", // Sun
    "راه", // Path
    "آسمان", // Sky
    "سیب", // Apple
    "خوب", // Good
    "کلمه", // Word
    "شب", // Night
    "نور", // Light
    "دست", // Hand
    "رود", // River
    "پر", // Full
    "خوشی", // Happiness
    "غم", // Sadness
    "لبخند", // Smile
    "بینش", // Vision
    "زاده", // Birth
    "قدرت", // Power
    "طلا", // Gold
    "خواسته", // Desire
    "هوا", // Air

    // Longer words (6 to 8 characters)
    "تعلیم", // Education
    "خوشحال", // Happy
    "دوستی", // Friendship
    "خانواده", // Family
    "زراعت", // Agriculture
    "سلامت", // Health
    "دانشگاه", // University
    "صلح", // Peace
    "فرهنگ", // Culture
    "توسعه", // Development
    "علم", // Knowledge
    "محیط زیست", // Environment
    "آزادی", // Freedom
    "شادی", // Joy
    "اعتماد", // Trust
    "اجتماعی", // Social
    "مبارزه", // Struggle
    "پیشرفت", // Progress
    "خواب", // Dream
    "قدرت", // Strength
    "طبیعت", // Nature
    "خلاقیت", // Creativity
    "سخت کوشی", // Hard work
    "مسئولیت", // Responsibility
    "همکاری", // Cooperation
    "موفقیت", // Success
    "موسیقی", // Music
    "وضعیت", // Situation
    "جاذبه", // Attractiveness
    "مثبت", // Positive
    "منفی", // Negative
  ],
  Dzongkha: [
    // Shorter words (3 to 5 characters)
    "ཆུ", // Water
    "དཀར", // White
    "ཕྱག", // Hand
    "བོད", // Tibet
    "མོ", // Moon
    "རྒན", // Old
    "རོགས", // Help
    "ཕར", // Path
    "ལོག", // Return
    "དབང", // Power
    "མཆོག", // Supreme
    "མཛོད", // Office
    "རང", // Self
    "དང", // And
    "བསོད", // Happy
    "མཆོད", // Offer
    "རིན", // Gold
    "རྩེ", // Fire
    "དགོངས", // Thought
    "རྐང", // Leg
    "འགོ", // Front
    "འབྲང", // Sweet
    "རྒེད", // Fruit
    "ཉལ", // Sleep
    "མོང", // Dream
    "བར", // Light
    "ཡང", // Again
    "ལྟ", // Look
    "ལག", // Instrument
    "ཁྱེད", // You

    // Longer words (6 to 8 characters)
    "བོད་པ", // Tibetan (person)
    "བཀྲ་ཤིས", // Blessing
    "རིགས་ཆུང་", // Young
    "ཕོ་བོད", // Father
    "སངས་པ", // Enlightenment
    "གཡུལ་འདུག", // Kingdom
    "བརྒྱབ་པ", // Power
    "སྐྱོངས", // Protection
    "ཕྱི་རོལ", // Surrounding
    "བསྟན་འཛིན", // Teaching
    "དབང་བསྟན", // Empowerment
    "བསོད་ནས", // Happiness
    "དབང་ཕྱག", // Authority
    "གཟུགས་ཡུག", // Appearance
    "དབང་དང", // Empower and control
    "བཟོད་མོ", // Life
    "ཕོ་བུང", // Young
    "གོ་བརྩག", // Agriculture
    "བོད་བདེ", // Tibetan happiness
    "མངོན་སྟོན", // Visibility
    "འཕུལ་འཛིན", // Abundance
    "རིན་བོད", // Gold Tibetan
    "གནམ་རོལ", // Motivation
    "སྐྱོན་རོལ", // Strength
    "བཟང་པ", // Good
    "ཆོས་ཡོན", // Philosophy
    "འཛིན་པ", // Observing
    "འཚམས་རིལ", // Persistence
    "དབང་འདུག", // Empowered
    "བརྒྱབ་པ", // Knowledge
    "དབང་ཡོད", // Control
  ],

  Korean: [
    // Shorter words (3 to 5 characters)
    "가", // Go
    "나", // I
    "그", // That
    "너", // You
    "물", // Water
    "사과", // Apple
    "고양이", // Cat
    "강아지", // Dog
    "책", // Book
    "사람", // Person
    "집", // House
    "하늘", // Sky
    "땅", // Land
    "바다", // Sea
    "불", // Fire
    "공", // Ball
    "길", // Road
    "나무", // Tree
    "산", // Mountain
    "별", // Star
    "달", // Moon
    "눈", // Eye
    "손", // Hand
    "코", // Nose
    "귀", // Ear
    "말", // Word
    "음악", // Music
    "아침", // Morning
    "저녁", // Evening
    "친구", // Friend

    // Longer words (6 to 8 characters)
    "학교", // School
    "컴퓨터", // Computer
    "사랑", // Love
    "행복", // Happiness
    "자유", // Freedom
    "문화", // Culture
    "경제", // Economy
    "사회", // Society
    "정치", // Politics
    "환경", // Environment
    "건강", // Health
    "교육", // Education
    "전통", // Tradition
    "관계", // Relationship
    "자연", // Nature
    "기술", // Technology
    "문학", // Literature
    "이해", // Understanding
    "대화", // Conversation
    "기억", // Memory
    "운동", // Exercise
    "정보", // Information
    "예술", // Art
    "연구", // Research
    "선택", // Choice
    "지혜", // Wisdom
    "신뢰", // Trust
    "개발", // Development
    "목표", // Goal
    "전략", // Strategy
  ],

  Persian: [
    // Shorter words (3 to 5 characters)
    "کتاب", // Book
    "دوست", // Friend
    "آب", // Water
    "نور", // Light
    "روز", // Day
    "شب", // Night
    "خورشید", // Sun
    "ماه", // Moon
    "گل", // Flower
    "باد", // Wind
    "خواب", // Sleep
    "عشق", // Love
    "سفر", // Travel
    "دست", // Hand
    "چشم", // Eye
    "پر", // Feather
    "قلب", // Heart
    "بزرگ", // Big
    "کوچک", // Small
    "زیبا", // Beautiful
    "غذا", // Food
    "شیر", // Milk
    "پول", // Money
    "دور", // Far
    "نزدیک", // Close
    "سخت", // Hard
    "راست", // Right
    "چپ", // Left
    "آرام", // Calm
    "شکست", // Defeat
    "پیروزی", // Victory

    // Longer words (6 to 8 characters)
    "فرهنگ", // Culture
    "محبت", // Affection
    "علمی", // Scientific
    "آزادگی", // Freedom
    "آینده", // Future
    "اجتماعی", // Social
    "مدرسه", // School
    "دولت", // Government
    "اقتصاد", // Economy
    "تحصیلات", // Education
    "دندان", // Tooth
    "کشور", // Country
    "کتابخانه", // Library
    "همسایه", // Neighbor
    "سخنرانی", // Speech
    "بیمارستان", // Hospital
    "سازمان", // Organization
    "توسعه", // Development
    "خوشحالی", // Happiness
    "مدیریت", // Management
    "فناوری", // Technology
    "محیط زیست", // Environment
    "آموزش", // Training
    "موسیقی", // Music
    "زبان", // Language
    "خدمات", // Services
    "رسانه", // Media
    "نظام", // System
    "مشارکت", // Participation
    "پیشرفت", // Progress
    "سازش", // Compromise
  ],
  Bhojpuri: [
    // Shorter words (3 to 5 characters)
    "जा", // Go
    "मैं", // I
    "तू", // You
    "हम", // We
    "बात", // Talk
    "घर", // House
    "पानी", // Water
    "सूरज", // Sun
    "चाँद", // Moon
    "फूल", // Flower
    "पेड़", // Tree
    "किताब", // Book
    "खाना", // Food
    "कुत्ता", // Dog
    "बिल्ली", // Cat
    "सड़क", // Road
    "नदी", // River
    "समय", // Time
    "आसमान", // Sky
    "दूर", // Far
    "नज़दीक", // Close
    "बड़ा", // Big
    "छोटा", // Small
    "खुश", // Happy
    "गुस्सा", // Angry
    "नया", // New
    "पुराना", // Old
    "सपना", // Dream
    "दर्द", // Pain
    "राजा", // King

    // Longer words (6 to 8 characters)
    "पढ़ाई", // Education
    "संगीत", // Music
    "खेलना", // Play
    "संबंध", // Relationship
    "अर्थव्यवस्था", // Economy
    "स्वास्थ्य", // Health
    "विकास", // Development
    "समाज", // Society
    "संस्कृति", // Culture
    "शिक्षा", // Learning
    "सफर", // Travel
    "व्यापार", // Business
    "समस्या", // Problem
    "सुरक्षा", // Safety
    "अनुभव", // Experience
    "खुशबू", // Fragrance
    "समझदारी", // Understanding
    "निर्णय", // Decision
    "प्रेमिका", // Girlfriend
    "मौका", // Opportunity
    "प्रगति", // Progress
    "खुशियाँ", // Joy
    "मित्रता", // Friendship
    "बोलचाल", // Conversation
    "आवाज़", // Voice
    "अवसर", // Chance
    "संपर्क", // Contact
    "विज्ञान", // Science
    "संदेश", // Message
    "विवरण", // Details
  ],

  Berber: [
    // Shorter words (3 to 5 characters)
    "ⴰⵣⵓⵍ", // Hello
    "ⵜⴰⵎ", // Mother
    "ⴱⴰⵃⴰⵔ", // Sea
    "ⵉⵏⴷⵓ", // Fire
    "ⵍⵃⵓⴱ", // Love
    "ⵏⵏⴰ", // Father
    "ⴷⴷⵓ", // Hand
    "ⵜⴰⵡ", // Water
    "ⵡⴰⵢ", // Eye
    "ⵓⵔ", // No
    "ⵉⵡⴻⵏ", // Time
    "ⵉⵍⴻⵍ", // Star
    "ⵙⴰⴼⵉ", // Pure
    "ⵙⵓⴽⵓ", // Good
    "ⴰⵎⵓⴷ", // Stone
    "ⵉⵡⴰⵍⵉ", // Wind
    "ⵜⵉⵣⵉ", // Mountain
    "ⵃⴻⵏⴰ", // Sky
    "ⵜⴰⵍ", // Tree
    "ⵉⵎⵓⴷ", // Heart
    "ⵅⵉⵍ", // Sun
    "ⵎⵉⵔ", // Place
    "ⵃⵓⵣ", // Life
    "ⴰⵣⴰ", // Man
    "ⵏⵏⵉ", // Woman
    "ⵓⵥⵓ", // Rock
    "ⵎⵉⵔ", // Soil
    "ⵓⵇⴻⵍ", // Road
    "ⴷⴰⵍ", // Child

    // Longer words (6 to 8 characters)
    "ⴰⴼⵓⵙ", // Finger
    "ⵜⴰⵎⵓⵔⵜ", // Land
    "ⵉⵏⵡⴻⵏ", // Language
    "ⴰⴷⵉⴱ", // Peace
    "ⵉⴼⵊⵉ", // Health
    "ⵙⴰⵃⵕⴰ", // Desert
    "ⵜⴰⴱⵔⴰⵜ", // Blessing
    "ⵉⴷⵓⵔⴰ", // Mountain
    "ⴰⵙⵉⵏⵜ", // Society
    "ⵜⴰⵣⵡⴰⵜ", // Gift
    "ⵏⵏⵉⵗⵉ", // Freedom
    "ⵉⵎⴰⵣⵉⵖⴻⵏ", // Berbers
    "ⵜⴰⵎⴰⵣⵉⵖⵜ", // Berber Language
    "ⵏⵉⴳⵔⴰ", // Tradition
    "ⵜⵉⵣⵉⴷ", // Respect
    "ⵜⵉⵎⵓⴽⴰ", // Community
    "ⴰⵎⵏⵉⵉ", // Trust
    "ⵉⴽⵍⵉⵙ", // Dialogue
    "ⵉⵏⵡⵉⵔ", // History
    "ⴰⴳⴳⵓ", // Journey
    "ⵏⴻⵢⴰ", // Family
    "ⵉⵎⴻⵔ", // Dream
    "ⵍⴰⵖⴰ", // Waterfall
    "ⵓⵎⵓⵙ", // Festival
    "ⴰⵉⵡⴻ", // Nature
    "ⴰⵎⵉⴷ", // Knowledge
    "ⵙⵓⵕⴰ", // Love
    "ⵉⵏⵡⴰ", // Childhood
  ],
  Amharic: [
    // Shorter words (3 to 5 characters)
    "ይህ", // This
    "እኔ", // I
    "አንተ", // You
    "ውሃ", // Water
    "ቤት", // House
    "ሰማይ", // Sky
    "ወንድ", // Man
    "ወንድም", // Brother
    "እህት", // Sister
    "ተምህር", // Learn
    "ታይ", // See
    "ሕይወት", // Life
    "ዝንባሌ", // Bird
    "ባለሙያ", // Expert
    "ወቅታዊ", // Current
    "ጊዜ", // Time
    "ዕቃ", // Item
    "ህይወት", // Existence
    "ነቃ", // Awake
    "ሴት", // Woman
    "ግሩም", // Wonderful
    "ጌታ", // Lord
    "አንበሳ", // Lion
    "የታይ", // Sign
    "መናታ", // Thank you
    "ዳንኳር", // Guardian

    // Longer words (6 to 8 characters)
    "ድምፅ", // Sound
    "መለኮት", // Heaven
    "የገንዘብ", // Financial
    "መምህር", // Teacher
    "ዕውቀት", // Knowledge
    "ጥበብ", // Wisdom
    "ትምህርት", // Education
    "ንቁ", // Sleep
    "የነበረ", // Existence
    "እርዳታ", // Assistance
    "አሰቃቂ", // Surprise
    "ምርጥ", // Best
    "እርግጥ", // Security
    "አዳራሽ", // Location
    "አሳማ", // Flag
    "ምንጭ", // Source
    "ወጥ", // Fall
    "እንግዳ", // Guest
    "በዓለም", // World
    "አስተያየት", // Observation
    "መምህር", // Instructor
    "ማህበረሰብ", // Community
    "መገኛ", // Solution
    "መጠን", // Quantity
    "መነሻ", // Beginning
    "አዲስ", // New
    "ዝርዝር", // List
    "ድንበር", // Border
    "አመት", // Year
  ],
  Yoruba: [
    // Shorter words (3 to 5 characters)
    "baba", // Father
    "iya", // Mother
    "ẹjẹ", // Blood
    "ara", // Body
    "oṣoo", // Moon
    "orun", // Heaven
    "iyawo", // Wife
    "ọrẹ", // Friend
    "omi", // Water
    "oko", // Husband
    "jọ", // Dance
    "ẹru", // Fear
    "ọna", // Path
    "ẹyẹ", // Bird
    "kò", // No
    "bẹ́ẹ̀", // Yes
    "gbigba", // Acceptance
    "ìbè", // There
    "ọdún", // Year
    "ilé", // House
    "fún", // For
    "ìwé", // Book
    "ṣọ́", // Watch
    "má", // Don’t
    "sùn", // Sleep
    "kòṣù", // Frown
    "ọlọ́run", // God
    "dá", // Fix

    // Longer words (6 to 8 characters)
    "ayé", // World
    "orukọ", // Name
    "ìdílé", // Family
    "dàgbà", // Grow
    "sáré", // Run
    "tọrọ", // Ask
    "ààrẹ", // President
    "iyàwó", // Bride
    "ọrẹ́rẹ", // Friend
    "àṣà", // Tradition
    "ìdàgbàsókè", // Development
    "ìjìnlẹ̀", // Depth
    "ìbáṣepọ̀", // Interaction
    "àgbàlagbà", // Elder
    "àtẹ́wọ́", // Affirmation
    "ìmọ̀", // Knowledge
    "ọdúnṣé", // Celebration
    "ìkọ́kọ́", // Secret
    "ìrìnàjò", // Journey
    "àkànṣe", // Creation
    "ìdàpọ̀", // Cooperation
    "àkóso", // Leadership
    "ìmọ̀tara-ẹni", // Self-awareness
    "àtúnyẹ́", // Renovation
    "ìfáṣiṣẹ́", // Employment
    "ìṣirò", // Calculation
    "ìrètí", // Hope
    "àlàyé", // Explanation
    "fífo", // Warning
    "ìbèèrè", // Demand
  ],

  Hausa: [
    // Shorter words (3 to 5 characters)
    "baba", // Father
    "iya", // Mother
    "ruwa", // Water
    "gida", // House
    "gari", // Town
    "hanya", // Way
    "suna", // Name
    "ƙasa", // Country
    "ƙofa", // Door
    "doki", // Horse
    "tashi", // Rise
    "ƙauna", // Love
    "baki", // Mouth
    "ƙaura", // Move
    "kaya", // Goods
    "ido", // Eye
    "mai", // Owner
    "duka", // All
    "kudi", // Money
    "babu", // None
    "karya", // Lie
    "yar", // Girl
    "saka", // Wear
    "fita", // Go out
    "soro", // Seek
    "tana", // She
    "maka", // Help

    // Longer words (6 to 8 characters)
    "dariya", // Laughter
    "muhimmanci", // Importance
    "sadarwa", // Communication
    "ilimi", // Knowledge
    "malamai", // Teachers
    "mahaifiyata", // My mother
    "shakatawa", // Relaxation
    "maraba", // Welcome
    "koyarwa", // Teaching
    "jihar", // State
    "ƙungiya", // Organization
    "zuciya", // Heart
    "tallafi", // Support
    "mazauni", // Resident
    "tsare-tsare", // Plans
    "karatu", // Reading
    "sanyin", // Coolness
    "ƙarfafa", // Strengthen
    "harkokin", // Affairs
    "safar", // Journey
    "tattalin", // Economy
    "iyali", // Family
    "ma'ana", // Meaning
    "kyautatawa", // Improvement
    "ta'aziyya", // Condolences
    "wasa", // Play
    "karfafawa", // Empowerment
    "hankali", // Wisdom
    "gwaninta", // Proficiency
    "lafiya", // Health
    "al'ada", // Culture
    "matar", // Woman
  ],
  Zulu: [
    // Shorter words (3 to 5 characters)
    "amanzi", // Water
    "indlu", // House
    "umntwana", // Child
    "umama", // Mother
    "ubaba", // Father
    "ubude", // Height
    "incwadi", // Book
    "inja", // Dog
    "ubisi", // Wax
    "inyanga", // Moon
    "ilanga", // Sun
    "imvula", // Rain
    "ithoni", // Idol
    "isihlahla", // Tree
    "inkulumo", // Talk
    "uphaphe", // Apple
    "ufudo", // Chili
    "umfula", // River
    "umgwaqo", // Road
    "nsukuzonke", // Daily
    "umculo", // Music
    "imbewu", // Seed
    "umhlabathi", // Soil
    "igama", // Word
    "umnyama", // Black
    "mhlophe", // White
    "isihlahla", // Flower
    "umfana", // Boy
    "lukhuni", // Hard
    "ngaphandle", // Outside

    // Longer words (6 to 8 characters)
    "imfundo", // Education
    "umfundi", // Student
    "isiko", // Culture
    "impumelelo", // Success
    "ukulima", // Farming
    "umphakathi", // Social
    "isayensi", // Science
    "umzimba", // Organization
    "umndeni", // Family
    "amasiko", // Cultural
    "impilo", // Health
    "upolitiki", // Politics
    "umhlangano", // Meeting
    "ubuhle", // Goodness
    "ubuchwepheshe", // Technical
    "ukuziphatha", // Morality
    "ukuzwana", // Sociability
    "uthando", // Love
    "ukuphepha", // Security
    "ulwazi", // Knowledge
    "imidlalo", // Games
    "inhlangano", // Institution
    "amathuba", // Potential
    "ukuziphatha", // Behavior
    "ingxoxo", // Discussion
  ],
};

export const generateRandomWords = (
  count: number,
  wordList: string[]
): string => {
  return Array.from({ length: count })
    .map(() => wordList[Math.floor(Math.random() * wordList.length)])
    .join(" ");
};
